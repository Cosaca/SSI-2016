...
rijndael::rijndael(bitset<8> texto[TAM]):
iBlock_(),
kBlock_(),
intermediateState_(),
oBlock_(),
...
void rijndael::initialization(void)
{
    iBlock_ = new bitset<8> [TAM];
    kBlock_ = new bitset<8> [TAM];
    intermediateState_ = new bitset<8> [TAM];
...
void rijndael::memoryRelease(void)
{
    delete [] iBlock_;
    iBlock_ = NULL;
    delete [] kBlock_;
...
int rijndael::pos(int i, int j, int n)
{
    return i*n+j;
}

void rijndael::addRoundKey(bitset<8> m1[TAM])
{
    for (int i = 0; i < TAM; i++)
    {
...
void rijndael::byteSub(bitset<8> m1[TAM])
{
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
...
void rijndael::keyExpansion(int inx)
{
    bitset<8> aux;
...
void rijndael::encryptionProcess(void)
{
    writeInput();
    addRoundKey(iBlock_);
...
bitset<8>* rijndael::get_cifrado(void)
{
